...
/* const contenedorLista = document.querySelector('[data-lists]');
const newListForm = document.querySelector('[data-new-list-form]');
const newListInput = document.querySelector('[data-new-list-input]');
const deleteListButton = document.querySelector('[data-delete-list-button]');
...
clearCompleteTasksButton.addEventListener('click', e=>{
    const selectedList = listas.find(list=>list.id === selectedListId);
...
function render(){
    ...
    if(selectedListId ==null){
        ...
        renderTaskCount(selectedList);
        clearElement(taskContainer);
        renderTaks(selectedList);
    }
  }

  function renderTaks(selectedList){
      selectedList.tasks.forEach(task=>{
          const taskElement = document.importNode(taksTemplate.content , true)
          const checkbox = taskElement.querySelector('input');
          checkbox.id = task.id;
...
deleteListButton.addEventListener('click',e=>{
 listas = listas.filter(list=>list.id !== selectedListId);
 selectedListId = null;
 saveAndRender(); 
});
